...
@dataclass
class FormingOrderBlock:
    """An OB whose impulse is visible, but whose fractal BOS is not closed yet."""
    ...
    @property
    def id(self) -> str:
        return f"{self.symbol}-{self.timeframe}-{self.direction}-{self.ob_time}-forming"
...
def find_forming_obs(
    df: pd.DataFrame,
    symbol: str,
    timeframe: str = "4H",
    displacement_atr: float = FORMING_DISPLACEMENT_ATR,
    window: int = FORMING_WINDOW,
) -> list[FormingOrderBlock]:
    """Find early OBs after displacement but before a close-based fractal BOS.

    The confirmed detector above remains the single source of confirmed OBs.
    This function only exposes the earlier ``forming`` stage.  A candidate is
    the opposite-colour base candle.  It becomes forming when price leaves its
    range by at least ``displacement_atr`` ATR within ``window`` closed candles.
    A still-unbroken confirmed fractal supplies the future BOS level.
    """
    prepared = prepare_indicators(df)
    if len(prepared) < ATR_PERIOD + 3:
        return []

    fractal_highs: list[int] = []
    fractal_lows: list[int] = []
    results: dict[str, FormingOrderBlock] = {}

    for index in range(len(prepared)):
        confirmed_index = index - 1
        if confirmed_index >= 1:
            if bool(prepared.at[confirmed_index, "fractal_high"]):
                fractal_highs.append(confirmed_index)
            if bool(prepared.at[confirmed_index, "fractal_low"]):
                fractal_lows.append(confirmed_index)

        close = float(prepared.at[index, "close"])
        fractal_highs = [i for i in fractal_highs if close <= float(prepared.at[i, "high"])]
        fractal_lows = [i for i in fractal_lows if close >= float(prepared.at[i, "low"])]

        first_candidate = max(0, index - window)
        for ob_index in range(first_candidate, index):
            atr_value = prepared.at[ob_index, "atr"]
            if pd.isna(atr_value) or float(atr_value) <= 0:
                continue
            atr = float(atr_value)
            open_ = float(prepared.at[ob_index, "open"])
            base_close = float(prepared.at[ob_index, "close"])
            high = float(prepared.at[ob_index, "high"])
            low = float(prepared.at[ob_index, "low"])

            if base_close < open_ and fractal_highs:
                fractal_index = fractal_highs[-1]
                bos_level = float(prepared.at[fractal_index, "high"])
                move = close - low
                if close > high and close <= bos_level and move >= displacement_atr * atr:
                    block = FormingOrderBlock(
                        symbol, timeframe, "bullish", open_, low,
                        _timestamp_label(prepared.at[ob_index, "ts"], timeframe),
                        _timestamp_label(prepared.at[index, "ts"], timeframe),
                        _timestamp_label(prepared.at[fractal_index, "ts"], timeframe),
                        bos_level, atr, move / atr,
                        ["opposite base candle", "bullish displacement", "BOS pending"],
                    )
                    results[block.id] = block

            if base_close > open_ and fractal_lows:
                fractal_index = fractal_lows[-1]
                bos_level = float(prepared.at[fractal_index, "low"])
                move = high - close
                if close < low and close >= bos_level and move >= displacement_atr * atr:
                    block = FormingOrderBlock(
                        symbol, timeframe, "bearish", high, open_,
                        _timestamp_label(prepared.at[ob_index, "ts"], timeframe),
                        _timestamp_label(prepared.at[index, "ts"], timeframe),
                        _timestamp_label(prepared.at[fractal_index, "ts"], timeframe),
                        bos_level, atr, move / atr,
                        ["opposite base candle", "bearish displacement", "BOS pending"],
                    )
                    results[block.id] = block
